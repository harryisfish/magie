...
2. 同一个 Terminal 可在 Web / Desktop / Mobile 多端同时显示；服务端只允许当前控制者输入和调整 PTY 尺寸，并阻止观察者 clear / restart / close、重启 inactive session 或改写受控 Terminal 状态。
...
- 扩展 Terminal RPC、服务端 `TerminalManager` 与共享客户端状态，加入控制权声明、控制状态投影、严格 owner-only write / resize，以及观察者 lifecycle mutation 门禁。
...
- Terminal 控制者以服务端认证 Session 标识；attach 不隐式抢占，只有 visible / focused 的活动视图可对无主 Terminal 发一次非 force claim，显式 Take control 才能抢占。
- `write` / `resize` 必须由当前 controller 发起；`clear` / `restart` / `close` 与 process-starting / size-changing `open` 可操作无主 Terminal，但不得跨越另一个 Session 的 controller，包括 process 已退出但 session / history 仍保留的状态。
- 多端显示不能让隐藏、失焦或观察端输入、resize 或上报后台尺寸改变操作者看到的 Terminal。
...
4. `apps/server/src/terminal/Manager.ts`、`apps/server/src/ws.ts`、`apps/server/src/auth/SessionStore.ts`、`apps/server/src/orchestration/Layers/ThreadDeletionReactor.ts`：控制状态、mutation 门禁、trusted cleanup 和断线释放。
5. `packages/client-runtime/src/state/terminal*.ts`、Web / Mobile Terminal 表面：控制状态投影、显式 claim、visible / focused 观察模式与 close 成功后本地收敛。
...
- Terminal server：运行 `Manager.test.ts`、Auth/WS 相关 focused tests，覆盖两个 attach 同时收到 snapshot/live、owner-only write/resize、running / inactive observer mutation 拒绝、无主 lifecycle mutation、thread-wide close 权限失败原子性、抢占、同一 Auth Session 多连接和最后断开释放。
...
- 未持有控制权的客户端不能写入或 resize；另一个 Session 已持有控制权时，observer 也不能 clear / restart / close、重启 inactive session 或删除保留历史；显式 claim 后新 owner 可操作，旧 owner 立即变为观察者。
...
| Terminal attach 与 control 分离                         | 多端默认都能看；只有 visible / focused 活动视图可领取无主 Terminal，输入和 resize 必须由 controller 发起。 |
...
| preflight 发现 observer 可 clear / restart / close |    1 | RPC 传真实 Auth Session；Manager 在 thread lock 内拒绝跨 owner mutation，并补原子 close 回归。         |
| hidden / unfocused Terminal 会静默 claim 和 resize |    1 | Web 以 `visible`、Mobile 以 navigation focus 门禁 claim / input / resize，并在恢复交互时上报最新尺寸。 |
| Right Panel 批量 close 先删除本地 surface          |    1 | 按每个 RPC 成功项精确移除 Terminal；失败项及等待期间新增的 split 保留。                                |
| inactive Terminal 可绕过 observer mutation 门禁    |    1 | controller 跨自然 exit 保留；clear / close / open 与 thread-wide close 对所有 session 统一校验 owner。 |
| 无主运行中 Terminal 的 authenticated `open` 不生效 |    1 | `currentController === undefined` 纳入允许条件，并补真实 Session 尺寸变更回归。                        |
